...
makeExampleXBSeqDataSet <- function( )
{
  ...
  XBSeqDataSet( m, conds )
}


XBSeqDataSetFromHTSeqCount <- function( sampleTable, directory="." )
{
  ...
  if( ncol(sampleTable) == 3 )
    XBSeqDataSet( tbl, sampleTable[,3] )
  else
    XBSeqDataSet( tbl, sampleTable[,-(1:2)] )
}
...
estimateRealcount<- function( counts, bgcounts ){
  if( nrow( as.matrix(counts) )!= nrow( as.matrix(bgcounts) ) ){
